...
// The middle of a turn, drawn two ways. Dev Mode draws the reasoning group as a
// box of its own, with the trace as children and the word "Reasoning" on the
// trigger. User Mode folds the whole middle into one row — the way Claude shows a
// turn at work — so its rules land on `.thought-*`, and the trace and the calls
// open beneath it. Both share the trigger's shimmer and the trace's words, which
// land one at a time instead of in whole paragraphs.
const REASONING_RULES = [
  ...
  ".thought-group{margin:8px 0}",
  ".thought-trigger{display:flex;align-items:center;gap:6px;width:100%;padding:0;border:0;background:transparent;color:var(--a-fg);cursor:pointer;text-align:left}",
  ".thought-dot{flex:none;width:5px;height:5px;border-radius:999px;background:var(--a-accent);animation:reasoning-pulse 1.6s ease-in-out infinite}",
  ...
  ".thought-tools > *{padding:4px 0;border-top:1px solid var(--a-border)}",
  ".thought-tools > *:first-child{border-top:0}",
  ...
  "@media (prefers-reduced-motion:reduce){.reasoning-trigger-label[data-active]{animation:none;-webkit-text-fill-color:currentColor;background-image:none}.reasoning-content,.thought-dot,.reasoning-word{animation:none}}",
  "",
].join("\n");
...
const TOOL_RULES = [
  ...
  ".tool-call-trigger{display:flex;width:100%;gap:8px;align-items:center;padding:4px 0;border:0;background:transparent;color:var(--a-muted);cursor:pointer;font:inherit;font-size:13px;text-align:left}",
